...
const login = handleSubmit(async (values) => {
  try {
    ...
    if (err.response && err.response.data) {
      ...
      alert('Ошибка входа: ' + err.response?.message || err.message);
    }
  }
})
    ...
    <section class="min-h-screen flex items-center justify-center">
      <div class="w-full max-w-md p-4 bg-white rounded-lg shadow-lg">
        <form @submit.prevent="login" autocomplete="off">
          ...
          <div class="m-3 border-t border-gray-200 mb-3"></div>
          <div class="relative">
          ...
          <div class="text-center">
            <p>Нет аккаунта? <a class="text-[#ec3606] hover:text-[#ec572f] font-medium" href="/"> Регистрация</a></p>
          </div>
        </form>
      </div>
    </section>
